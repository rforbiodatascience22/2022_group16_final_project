...
data %>% 
  mutate(income_nor = normalize(data$income)) %>% 
  ggplot(aes(x = re, 
             y = income_nor, stat_binline(position = "density"))) + 
  geom_col() + 
  #scale_y_continuous(labels = trans_format(format = ))
  facet_grid(dx ~ tx) + 
...
### Creating correlation matrix using corr(). 
#### The default is pearson correlation coefficient which measures the linear dependence between two variables.
#### Non-numeric variables were deselected. 
#### use = "complete.obs" handles missing values by case-wise deletion.

```{r}
corr <- data %>% 
  select( -c(seqn, sex, sex_class, re, tx, dx, bmi_class)) %>% 
  cor( method = "pearson", 
       use = "complete.obs" )
```

### Plotting correlations of the above multiple numerix variables using corrplot() function, which creates a graphical display of a correlation matrix, highlighting the most correlated variables in a data table.

#### type = "upper": only shows upper half of the plot. 
#### order = "hclust": reorder according to the correlation coefficient
...
```{r}
  cor(data[, c("bmi", "gh", "albumin", "bun")], method = "pearson", use = "complete.obs") %>% 
  pairs.panels()
```
